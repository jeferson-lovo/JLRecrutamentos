...
<tr>
    ...
    <td>{{ $cd->uf_cidade }}</td>
    <td>
        <form action="{{ route('cidades.destroy', $cd->id)}}" method="post">
            @csrf
            @method('DELETE')
            ...
            <a class="btn btn-secondary btn-sm active" href="{{ route('cidades.edit', $cd->id)}}">
                Editar
            </a>
        </form>
    </td>
...
@endforeach
    ...
    <div class="btn">
        ...
        <a href="{{ route('cidades.create') }}" class="btn" type="submit">Nova Cidade</a>
    </div>
